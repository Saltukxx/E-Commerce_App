"""Deploy marketplace seller/trust features + Prisma migration to production."""
...
def main() -> None:
    ...
    remote_tar = "/tmp/marketplace-trust-sync.tar.gz"
    ...
    steps = [
        ...
        (
            f"cd {REMOTE_DIR} && docker compose -f docker-compose.prod.server.yml --env-file .env "
            f"exec -T api npx prisma migrate status"
        ),
    ]
    for cmd in steps:
    ...
    checks = [
        ...
        "curl -sf -o /dev/null -w 'stores:%{http_code} ' http://127.0.0.1:3001/api/v1/stores",
        "curl -sf -o /dev/null -w 'store_detail:%{http_code} ' http://127.0.0.1:3001/api/v1/stores/durmusbaba",
        "curl -sf -o /dev/null -w 'offers:%{http_code} ' 'http://127.0.0.1:3001/api/v1/products/slug/test/offers'",
    ]
    ...
    client.close()
    print("Marketplace trust deploy complete.", flush=True)
